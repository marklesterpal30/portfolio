...
const SocialConnectButton = () => {
	...
	const socialLinks = [
		{
			name: "Facebook",
			url: "https://facebook.com/marklester",
			icon: (
		...
		{
			name: "Instagram",
			url: "https://instagram.com/marklester",
			icon: (
				<svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
					<path
            ...
            4.948.2 4.358 2.618 6.78 6.98 
            6.98 1.281.058 1.689.072 4.948.072 
            ...
            1.44 1.441 1.44c.795 0 1.439-.645 
		...
		{
			name: "GitHub",
			url: "https://github.com/marklester",
			icon: (
				<svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
					<path
						d="M12 0c-6.626 0-12 
            5.373-12 12 0 5.302 3.438 9.8 
            ...
            0 0 1.008-.322 3.301 
            1.23.957-.266 1.983-.399 3.003-.404 
            1.02.005 2.047.138 
	...
	useEffect(() => {
		const handleScroll = () => {
			if (window.scrollY === 0) {
				...
				setIsOpen(false); // close when scrolled down
			}
		};
	...
	return (
		<div className="fixed top-4 right-4 z-50">
			...
			<motion.button
				...
				className="bg-dark-blue hover:bg-blue-700 text-white p-3 rounded-full shadow-lg transition-colors duration-200 flex items-center space-x-2"
			...
			<AnimatePresence>
				{isOpen && (
					<motion.div
						...
						className="absolute top-full right-0 mt-2 bg-white rounded-lg shadow-xl border border-gray-200 p-2 min-w-[180px]"
					>
						<div className="text-sm font-semibold text-gray-700 px-3 py-2 border-b border-gray-100">
							Connect with me
						</div>
						<div className="space-y-1 mt-2">
							{socialLinks.map((social) => (
								<motion.a
									...
									className={`flex items-center space-x-3 px-3 py-2 rounded-md text-white text-sm font-medium transition-colors duration-200 ${social.color}`}
